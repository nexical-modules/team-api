...
import { GET } from '../../../../../src/pages/api/team-api-key/[id]';
...
describe('TeamApiKey API - GET ../../../../../src/pages/api/team-api-key/[id]', () => {
  ...
  it('should call TeamApiKeyService and return success', async () => {
    const query = ['GET', 'DELETE'].includes('GET'.toUpperCase())
      ? `?name=${encodeURIComponent(String('test'))}&hashedKey=${encodeURIComponent(String('test'))}&prefix=${encodeURIComponent(String('test'))}&lastUsedAt=${encodeURIComponent(String(new Date().toISOString()))}&expiresAt=${encodeURIComponent(String(new Date().toISOString()))}&teamId=${encodeURIComponent(String('test'))}`
      : '';
    ...
    const serviceMethod =
      'GET'.toLowerCase() === 'post'
        ? 'create'
        : 'GET'.toLowerCase() === 'get'
          ? '../../../../../src/pages/api/team-api-key/[id]'.includes('[id]')
            ? 'get'
            : 'list'
          : 'GET'.toLowerCase() === 'put'
            ? 'update'
