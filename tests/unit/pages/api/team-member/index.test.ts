...
import { GET } from '../../../../../src/pages/api/team-member/index';
...
describe('TeamMember API - GET ../../../../../src/pages/api/team-member/index', () => {
  beforeEach(() => {
    vi.resetAllMocks();
    vi.mocked(ApiGuard.protect).mockResolvedValue(undefined);
  });

  it('should call TeamMemberService and return success', async () => {
    const query = ['GET', 'DELETE'].includes('GET'.toUpperCase())
      ? `?role=${encodeURIComponent(String('TEAM_OWNER'))}&userId=${encodeURIComponent(String('test'))}&teamId=${encodeURIComponent(String('test'))}`
      : '';
    ...
    const serviceMethod =
      'GET'.toLowerCase() === 'post'
        ? 'create'
        : 'GET'.toLowerCase() === 'get'
          ? '../../../../../src/pages/api/team-member/index'.includes('[id]')
            ? 'get'
            : 'list'
          : 'GET'.toLowerCase() === 'put'
            ? 'update'
    ...
    vi.mocked((TeamMemberService as Record<string, unknown>)[serviceMethod]).mockResolvedValue({
      success: true,
      data: { id: 'test-id' },
