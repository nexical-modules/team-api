...
import { POST } from '../../../../../../../src/pages/api/team-member/invitations/[id]/resend';
vi.mock('../../../../../../../src/actions/resend-invitation-team-member');
vi.mock('@/lib/api/api-guard');

describe('TeamMember API - POST ../../../../../../../src/pages/api/team-member/invitations/[id]/resend', () => {
  beforeEach(() => {
    vi.resetAllMocks();
    vi.mocked(ApiGuard.protect).mockResolvedValue(undefined);
  });

  it('should call ResendInvitationTeamMemberAction and return success', async () => {
    const query = ['GET', 'DELETE'].includes('POST'.toUpperCase())
      ? `?invitationId=${encodeURIComponent(String('test'))}`
      : '';
    ...
    vi.mocked(ResendInvitationTeamMemberAction.run).mockResolvedValue({
      success: true,
      data: { id: 'test-id' },
