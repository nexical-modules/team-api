...
import { TeamApiKeyService } from '../../../src/actions/../services/team-api-key-service';
...
vi.mock('../../../src/actions/../services/team-api-key-service');

describe('CreateTeamApiKeyAction', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('should be defined', () => {
    expect(CreateTeamApiKeyAction).toBeDefined();
  });

  it('should run (smoke test)', async () => {
    const teamApiKeyServiceMock = vi.mocked(TeamApiKeyService);
    commonServiceMethods.forEach((m) => {
      if ((teamApiKeyServiceMock as unknown as Record<string, vi.Mock>)[m])
        (teamApiKeyServiceMock as unknown as Record<string, vi.Mock>)[m].mockResolvedValue({
          success: true,
          data: { id: '1' },
        });
    });
    ...
    const result = await CreateTeamApiKeyAction.run(
      {
        id: '1',
        userId: 'user-1',
        teamId: 'team-1',
  ...
  it('should call TeamApiKeyService on success', async () => {
    const mockData = { id: '1' };
  ...
  it('should return error when TeamApiKeyService fails', async () => {
    const serviceMock = vi.mocked(TeamApiKeyService);
    commonServiceMethods.forEach((m) => {
