...
export class TeamHooks {
  static init() {
    ...
    HookSystem.on(
      'team.beforeCreate',
      async (data: Prisma.TeamCreateInput & { ownerId?: string }) => {
    ...
    HookSystem.filter('team.list', async (teams: Team[]) => {
    ...
    HookSystem.on('team.beforeUpdate', async (data: Prisma.TeamUpdateInput & { id?: string }) => {
  ...
  static initListFilter() {
    HookSystem.filter(
      'team.beforeList',
      async (params: Prisma.TeamFindManyArgs & { actor?: ApiActor }) => {
