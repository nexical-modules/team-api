...
import type { CreateTeamApiKeyDTO } from "@modules/team-api/src/sdk";
// GENERATED CODE - DO NOT MODIFY
export const POST = defineApi(
  async (context) => {
    // 1. Body Parsing (Input)
    const body = (await context.request.json()) as CreateTeamApiKeyDTO;
    const query = Object.fromEntries(new URL(context.request.url).searchParams);

    // 2. Hook: Filter Input
    const input: CreateTeamApiKeyDTO = await HookSystem.filter(
