...
import type { AcceptInvitationDTO } from "@modules/team-api/src/sdk";
// GENERATED CODE - DO NOT MODIFY
export const POST = defineApi(
  async (context) => {
    // 1. Body Parsing (Input)
    const body = (await context.request.json()) as AcceptInvitationDTO;
    const query = Object.fromEntries(new URL(context.request.url).searchParams);

    // 2. Hook: Filter Input
    const input: AcceptInvitationDTO = await HookSystem.filter(
    ...
    // 4. Action Execution
    const result = await AcceptInvitationTeamMemberAction.run(
      combinedInput,
      context,
    );
    ...
    const filteredResult = await HookSystem.filter(
      "teamMember.acceptInvitation.output",
      result,
    );
