...
import type { CreateTeamDTO } from "@modules/team-api/src/sdk";
import { CreateTeamAction } from "@modules/team-api/src/actions/create-team";
// GENERATED CODE - DO NOT MODIFY
export const POST = defineApi(
  async (context) => {
    // 1. Body Parsing (Input)
    const body = (await context.request.json()) as CreateTeamDTO;
    const query = Object.fromEntries(new URL(context.request.url).searchParams);

    // 2. Hook: Filter Input
    const input: CreateTeamDTO = await HookSystem.filter(
